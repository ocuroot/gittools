...
func CreateTestRemoteRepo(baseName string) (repoPath string, cleanup func(), err error) {
	...
	// Initialize bare repository - use master branch for compatibility with older Git versions
	_, err = client.InitBare(repoPath, "master")
	if err != nil {
		cleanup()
	...
	if err := tempRepo.Push("origin", "master"); err != nil {
		cleanup()
